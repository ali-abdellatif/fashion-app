...
<!-- breadcrumb -->
            ...
            <div class="container">
                <div class="tf-breadcrumb-wrap d-flex justify-content-between flex-wrap align-items-center">
                    <div class="tf-breadcrumb-list">
                        <a href="{{ route('home') }}" class="text">{{ __('Home') }}</a>
                                ...
                                <div class="thumbs-slider">
                                    <div dir="ltr" class="swiper tf-product-media-thumbs other-image-zoom"
                                        ...
                                        <div class="swiper-wrapper stagger-wrap">
                                            ...
                                            @php
                                                $enName   = $img->getTranslation('color_name', 'en', false);
                                                $colorKey = $enName ? \Illuminate\Support\Str::slug($enName) : 'img-' . $img->id;
                                            @endphp
                                            <div class="swiper-slide stagger-item" data-color="{{ $colorKey }}">
                                                <div class="item">
                                                    <img class="lazyload"
                                                        data-src="{{ $img->url }}"
                                                        src="{{ $img->url }}"
                                    ...
                                    <div dir="ltr" class="swiper tf-product-media-main" id="gallery-swiper-started">
                                        <div class="swiper-wrapper">
                                            ...
                                            @php
                                                ...
                                                $colorKey = $enName ? \Illuminate\Support\Str::slug($enName) : 'img-' . $img->id;
                                            @endphp
                                            <div class="swiper-slide" data-color="{{ $colorKey }}">
                                                <a href="{{ $img->url }}" target="_blank" class="item" data-pswp-width="770px" data-pswp-height="1075px">
                                ...
                                <div class="tf-product-info-list other-image-zoom">
                                    ...
                                    <div class="tf-product-info-price">
                                        @if($product->sale_price)
                                            <div class="price-on-sale">{{ number_format($product->sale_price, 2) }} EGP</div>
                                            <div class="compare-at-price">{{ number_format($product->price, 2) }} EGP</div>
                                        @else
                                            <div class="price">{{ number_format($product->price, 2) }} EGP</div>
                                        @endif
                                    </div>

                                    @php $colorImages = $product->colorImages(); @endphp
                                    <div class="tf-product-info-variant-picker">
                                        @if($colorImages->count())
                                        <div class="variant-picker-item">
                                            <div class="variant-picker-label">
                                                {{ __('Color') }}: <span class="fw-6 variant-picker-label-value value-currentColor">{{ $colorImages->first()->color_name }}</span>
                                            </div>
                                            <div class="variant-picker-values">
                                                @foreach($colorImages as $i => $img)
                                                @php
                                                    $enName   = $img->getTranslation('color_name', 'en', false);
                                                    $colorKey = $enName ? \Illuminate\Support\Str::slug($enName) : 'img-' . $img->id;
                                                @endphp
                                                <input id="color-{{ $colorKey }}" type="radio" name="color1" data-id="{{ $img->id }}" {{ $i === 0 ? 'checked' : '' }}>
                                                <label class="hover-tooltip radius-60 color-btn" for="color-{{ $colorKey }}"
                                                    data-color="{{ $colorKey }}" data-value="{{ $img->color_name }}">
                                                    <span class="btn-checkbox" style="background-color: {{ $img->color_hex }};"></span>
                                        ...
                                        <div class="variant-picker-item">
                                            <div class="d-flex justify-content-between align-items-center">
                                                <div class="variant-picker-label">
                                                    {{ __('Size') }}: <span class="fw-6 variant-picker-label-value">{{ $product->sizes->first()->name }}</span>
                                                </div>
                                            </div>
                                            <div class="variant-picker-values">
                                                @foreach($product->sizes as $i => $size)
                                                <input type="radio" name="size1" id="size-{{ $size->id }}" data-id="{{ $size->id }}" {{ $i === 0 ? 'checked' : '' }}>
                                                <label class="style-text size-btn" for="size-{{ $size->id }}" data-value="{{ $size->name }}">
                                                    <p>{{ $size->name }}</p>
                                                </label>
                                                @endforeach
                                            </div>
                                    ...
                                    <div class="tf-product-info-quantity">
                                        <div class="quantity-title fw-6">{{__('site.quantity')}}</div>
                                    ...
                                    <div class="tf-product-info-buy-button"
                                        ...
                                        <form>
                                            <button type="button"
                                                class="tf-btn btn-fill justify-content-center fw-6 fs-16 flex-grow-1 animate-hover-btn pd-btn-add-to-cart">
                                                <span>{{ __('site.add-to-cart') }} -&nbsp;</span>
                                                <span class="pd-total-price">{{ number_format($product->sale_price ?? $product->price, 2) }} EGP</span>
                                            </button>
                                            <div class="w-100">
                                                <button type="button" class="btns-full pd-btn-buy-now">{{ __('site.buy-now') }}</button>
                                            </div>
                                        </form>
                                    </div>
                    ...
                    <div class="tf-height-observer w-100 d-flex align-items-center">
                        <div class="tf-sticky-atc-product d-flex align-items-center">
                            <div class="tf-sticky-atc-img">
                                ...
                                <img class="lazyload"
                                    data-src="{{ $product->primaryImage()->url }}"
                                    ...
                                    <select class="tf-select">
                                        ...
                                        @if($colors->count() && $sizes->count())
                                            @foreach($colors as $color)
                                                ...
                                                <option>{{ $color->color_name }} / {{ $size->name }} - {{ number_format($price, 2) }} EGP</option>
                                                @endforeach
                                            @endforeach
                                        @elseif($sizes->count())
                                            @foreach($sizes as $size)
                                            <option>{{ $size->name }} - {{ number_format($price, 2) }} EGP</option>
                                            @endforeach
                                        @else
                                            <option>{{ number_format($price, 2) }} EGP</option>
                                        @endif
                                    </select>
                                </div>
        ...
        <section class="flat-spacing-1 pt_0">
            <div class="container">
                <div class="flat-title">
                    <span class="title">{{ __('site.related-product') }}</span>
                </div>
                    ...
                    <div dir="ltr" class="swiper tf-sw-product-sell wrap-sw-over" data-preview="4" data-tablet="3"
                        ...
                        <div class="swiper-wrapper">
                            ...
                            <div class="swiper-slide">
                                @include('site.partials.product-card', ['product' => $relProduct])
                            </div>
                            @endforeach
                        </div>
                    </div>
        ...
        <section class="flat-spacing-4 pt_0">
            <div class="container">
                <div class="flat-title">
                    <span class="title">{{ __('site.recently-viewed') }}</span>
                </div>
                <div class="hover-sw-nav hover-sw-2">
                    <div dir="ltr" class="swiper tf-sw-recent wrap-sw-over" data-preview="4" data-tablet="3"
                        ...
                        <div class="swiper-wrapper">
                            ...
                            <div class="swiper-slide">
                                @include('site.partials.product-card', ['product' => $recentProduct])
                            </div>
                            @endforeach
                        </div>
                    </div>
...
@push('scripts')
    <script src="{{ asset('site/js/zoom.js') }}"></script>
    <script>
    (function () {
        var wrap      = document.querySelector('[data-product-id]');
        var unitPrice = parseFloat(wrap.getAttribute('data-product-price'));

        function getSelected() {
            var colorEl = document.querySelector('input[name="color1"]:checked');
            var sizeEl  = document.querySelector('input[name="size1"]:checked');
            var qtyEl   = document.querySelector('.quantity-product');
            return {
        ...
        function updateTotalPrice() {
            var qty  = parseInt(document.querySelector('.quantity-product')?.value) || 1;
            var span = document.querySelector('.pd-total-price');
            if (span) span.textContent = (unitPrice * qty).toFixed(2) + ' EGP';
        }

        document.addEventListener('input', function (e) {
            if (e.target.classList.contains('quantity-product')) updateTotalPrice();
        });
        ...
        function doAdd(redirectAfter) {
            @auth('customer')
            var sel       = getSelected();
            var productId = wrap.getAttribute('data-product-id');
            var btn       = redirectAfter
                ? document.querySelector('.pd-btn-buy-now')
                : document.querySelector('.pd-btn-add-to-cart');
            window.addToCart(productId, sel.colorId, sel.sizeId, sel.qty, btn);
            if (redirectAfter) {
                setTimeout(function () { window.location.href = '{{ route("cart.index") }}'; }, 700);
            }
            @else
        ...
        document.querySelector('.pd-btn-add-to-cart')?.addEventListener('click', function () { doAdd(false); });
        document.querySelector('.pd-btn-buy-now')?.addEventListener('click', function () { doAdd(true); });
    })();
    </script>
@endpush